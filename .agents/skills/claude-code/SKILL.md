...
Safety override for this local install: do not use `--dangerously-skip-permissions`
or accept its bypass dialog by default. Prefer `--allowedTools`, `--disallowedTools`,
`--permission-mode default`, and `--max-turns`. Only use permission bypass when the
user explicitly requests it for a trusted workspace and understands that it
auto-approves file writes, shell commands, and network access.
```
...
```
# Launch with bounded permissions
terminal(command="tmux send-keys -t claude-work 'claude -p \"your task\" --allowedTools Read,Edit --max-turns 10' Enter")
...
**Note:** After the first trust acceptance for a directory, the trust dialog won't appear again. Treat the permissions bypass dialog as an explicit-risk prompt and leave it at the default "No, exit" unless the user approved bypass.
...
| Flag | Effect |
|------|--------|
| `--dangerously-skip-permissions` | Auto-approve ALL tool use (file writes, bash, network, etc.); do not use unless the user explicitly approved this risk |
...
Local safety override: do not accept `--dangerously-skip-permissions` prompts
unless the user explicitly approved bypass for a trusted workspace. Use scoped
tool allow-lists and turn/budget caps by default.
